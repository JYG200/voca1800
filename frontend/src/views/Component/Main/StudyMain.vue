<template>
  <main id="study-main">
    <div class="card-container">
      <!-- next card -->
      <div class="word-card next-card" v-show="words.length > 1" :style="nextCardStyle">
...
const handleSwipeOut = (dir) => {
  ...
  setTimeout(() => {
    ...
    // ✅ 마지막 1장이면 entering 스킵
    const hasCard = words.value.length > 0
    const isLastOne = words.value.length === 1

    animState.value = hasCard ? (isLastOne ? 'idle' : 'entering') : 'idle'
    frontPreview.value = words.value[0] ?? null

    if (!isLastOne && hasCard) {
...
// ✅ 드래그 글로우는 card-inner에만 적용
const cardInnerStyle = computed(() => {
  const base = { boxShadow: 'none' }
  if (animState.value !== 'dragging') return base

  const x = dragX.value
  const p = Math.min(1, Math.abs(x) / 130)

  const glowColor =
    x < 0 ? `rgba(34, 197, 94, ${p * 0.7})` : `rgba(239, 68, 68, ${p * 0.7})`

  return {
    boxShadow: `0 0 ${p * 40}px ${glowColor}`,
  }
})
