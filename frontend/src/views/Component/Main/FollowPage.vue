        ...
        <div class="header">
            ...
            <template v-if="isSearchMode">
                <div class="search-bar">
                    ...
                    <input ref="searchInputEl" class="search-input" placeholder="이름으로 검색" v-model="searchQuery" />
        ...
        <div class="list">
            ...
            <template v-if="isSearchMode">
                ...
                <div v-for="u in searchResults" :key="u.id" class="user-row">
                    ...
                    <button class="action-btn" :class="isFollowing(u.id) ? 'following' : 'follow'"
            ...
            <template v-else>
                ...
                <div v-for="u in activeList" :key="u.id" class="user-row">
                    <div class="avatar-wrap">
                        <div class="avatar-fallback" :style="{ background: u.avatarBg }">
                            {{ u.name.slice(0, 1).toUpperCase() }}
                        </div>
                    ...
                    <!-- ✅ 팔로워 탭: 맞팔 여부로 분기 -->
                    <button v-if="activeTab === 'followers'" class="action-btn"
                        :class="isFollowing(u.id) ? 'following' : 'follow'" @click="toggleFollow(u)">
                        {{ isFollowing(u.id) ? '팔로잉' : '맞팔로우' }}
                    </button>

                    <!-- ✅ 팔로잉 탭: 항상 팔로잉(회색) -->
                    <button v-else class="action-btn following" @click="toggleFollow(u)">
                        팔로잉
                    </button>
...
function toggleFollow(user) {
  if (isFollowing(user.id)) {
    // ✅ 언팔 시 확인
    if (!confirm(`${user.name} 님을 언팔로우하시겠어요?`)) return
